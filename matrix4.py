    ...
    def construct(self):
        ...
        for i, row in enumerate(matrix):
            ...
            for j, cell in enumerate(row):
                ...
                cell_rect = Rectangle(width=0.7, height=0.7, fill_opacity=0, stroke_opacity=1, stroke_width=1, color=WHITE)
        ...
        for i, row in enumerate(cells):
            for j, cell in enumerate(row):
                fill = cell[1].copy()
                fill.set_fill(WHITE, opacity=1)
                fill_animations.append(FadeIn(fill))
                self.add(fill)
                self.play(*fill_animations, run_time=0.1)
